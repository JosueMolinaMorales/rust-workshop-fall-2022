...
fn main() {
    ...
    loop {
        ...
        handle_user_guess(&mut number_of_guesses, random_number);
        // Check to see if the user would like to play again
        if handle_play_again() { continue; } else { break; }
    }

    println!("Thanks for playing {}! You played {number_of_games_played} times!", name_of_user.trim_end());
}

fn handle_user_guess(number_of_guesses: &mut u32, random_number: u32) {
    loop {
